...
public class ScoreServiceJdbc implements ScoreService{
    ...
    @Override
    public void addScore(ScoreNew score) {
    ...
    @Override
    public List<ScoreNew> getBestScore(String game) {
        ...
        )
        {
            ...
            try(var rs = statement.executeQuery()){
                var scores= new ArrayList<ScoreNew>();
                while(rs.next()) {
                    scores.add(new ScoreNew(rs.getString(1), rs.getString(2), rs.getInt(3), rs.getTimestamp(4)));
                }
                return scores;
            }
        }catch (SQLException e) {
